...
class PowerPointShowController:
    ...
    def goto(self, number):
        ...
        Note: In standalone mode, uses keyboard simulation to navigate.
        """
        if not self.launched:
            print("Warning: Slideshow not launched")
            return
        
        # In standalone mode, we need to use keyboard simulation
        # Since we don't have programmatic control
        try:
            import pyautogui
            import time
            
            # Calculate how many times to press keys
            # Go to slide 1 first (Home key), then navigate
            pyautogui.press('home')
            time.sleep(0.2)
            
            # Navigate to target slide (press Page Down number-1 times)
            for _ in range(number - 1):
                pyautogui.press('pagedown')
                time.sleep(0.1)
                
        except ImportError:
            print("Warning: pyautogui not installed. Cannot navigate slides.")
