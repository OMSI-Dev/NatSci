...
class sosppEngine:
    def __init__(self,ip,port,pp,pp_dictionary,database,mode,config,verbose):        
        self.runme = True
        self.ip = ip
        self.port = port
        self.pp = pp
    ...
    def __init_nowSocket__(self,attempts=None,debug=False):
        ...
        while self.runme:
            try:
                nowSocket.connect(("localhost", nowPort))        
                break
            except socket.error:
                if debug:
                    print ("NETWORK START FAIL")
    ...
    def run(self):
        ...
        while self.runme:
            ...
            if name.strip() in self.pp_dictionary.keys():
                slide_nums = self.pp_dictionary[name]
            else:
                slide_nums = [1]
                if self.verbose:
                    print( "Name Not Found in Database")
            if self.mode and slide_nums == [1] and len(name.strip())>0:
                total_slides += 1
                if self.verbose:
                    print ("Adding ",str(name)," as slide ",str(total_slides))
                self.pp.create_slide(name,total_slides)
                success = add_name_file(self.database,total_slides,name)
                self.pp_dictionary[name]=total_slides
                

            elif not self.mode:
                if not currentslide in slide_nums:
                    if self.verbose:
                        print ("Changing to Slide:" + str(slide_nums[0]))
                    slides_p_clip = len(slide_nums)
                    time_on_slide = 0
                    index_slide = 0
                    self.pp.goto(slide_nums[0])
                    currentslide = slide_nums[0]
                elif time_on_slide >= int(self.config["slide_timer"])/2.0:
                    #print "Changing . . . "
                    if slides_p_clip > 1:
