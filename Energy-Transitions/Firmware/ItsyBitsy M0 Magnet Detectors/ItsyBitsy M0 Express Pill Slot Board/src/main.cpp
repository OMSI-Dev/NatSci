...
// ── Debug Configuration ──────────────────────────────────────────────────────
#define ENABLE_SERIAL_DEBUG false  // Set to false for production to prevent Serial blocking
#define SERIAL_BAUD 115200

// Helper macros for conditional serial output - completely removed when debug disabled
#if ENABLE_SERIAL_DEBUG
  #define DEBUG_PRINT(x) Serial.print(x)
  #define DEBUG_PRINTLN(x) Serial.println(x)
  #define DEBUG_IF(code) code
#else
  #define DEBUG_PRINT(x)       // No-op
  #define DEBUG_PRINTLN(x)     // No-op
  #define DEBUG_IF(code)       // No-op - code block removed
#endif
...
// PER-SENSOR THRESHOLDS: Each Hall sensor has slightly different baseline characteristics
// S1 (middle) runs ~150-200 ADC units higher than S2/S3 due to component variance
//
// ⚠️ THRESHOLD TUNING GUIDE:
//   If a sensor reads UNCERTAIN (X) when it should read SOUTH (S):
//     → Increase that sensor's SOUTH_HIGH value
//   If a sensor reads UNCERTAIN (X) when it should read NORTH (N):
//     → Decrease that sensor's NORTH_LOW value
//   Use serial command 's' to see RAW ADC values and adjust accordingly
//
// Threshold format: [S1_middle, S2_left, S3_right]
const uint16_t THRESHOLD_SOUTH_LOW[3]  = {1900, 1650, 1550};  // S1 raised to 1900, S2 raised to 1650 (observed 1560-1579)
const uint16_t THRESHOLD_SOUTH_HIGH[3] = {2000, 1950, 1950};  // S1 set to 2000 (100 unit hysteresis)
const uint16_t THRESHOLD_NORTH_LOW[3]  = {2500, 2400, 2400};  // Raised above baseline (observed empty: S1~2400, S2~2300, S3~2280)
const uint16_t THRESHOLD_NORTH_HIGH[3] = {2700, 2550, 2550};  // Entry threshold for NORTH
// S1 UNCERTAIN range: 2000-2500 (500 units)
// S2/S3 UNCERTAIN range: 1950-2400 (450 units)
...
#define LOCK_MISMATCH_DEBOUNCE_MS 150  // Time a mismatch must persist to trigger reset
                                        // (filters ADC noise, prevents false unlocks from noisy power supply)
// #define LOCK_FAILSAFE_TIMEOUT_MS 5000 // REMOVED - was causing false unlocks on correctly registered pieces
#define LOCK_NOCHANGE_TIMEOUT_MS 3000 // Force unlock if ADC values haven't changed for 3s
...
struct SensorTrack
{
  ...
  uint16_t lockedRaw;     // Raw ADC value when locked (for slow removal detection)
};
...
// ── Results State Lock (prevents LED flickering) ─────────────────────────────
bool lockedResultsRegistered = false;  // Locked registration state for RESULTS
bool lockedResultsCorrect = false;     // Locked correctness for RESULTS
...
// ── I2C Communication ────────────────────────────────────────────────────────
volatile uint8_t txBuf[14] = {0, 0, 0, 0, 0x08, 0, 0, 0, 0, 0, 0, 0, 0, 0};
 ...
 * Uses per-sensor thresholds since each Hall sensor has different characteristics.
 * 
 * @param raw Current ADC reading
 * @param lastReading Previous polarity state (for hysteresis)
 * @param sensorIndex Sensor number (0=S1/middle, 1=S2/left, 2=S3/right)
 */
Polarity classifyWithHysteresis(uint16_t raw, Polarity lastReading, uint8_t sensorIndex)
{
  if (lastReading == POL_SOUTH)
  {
    // Currently SOUTH: need to exceed high threshold to leave
    if (raw < THRESHOLD_SOUTH_HIGH[sensorIndex])
      return POL_SOUTH;
    if (raw > THRESHOLD_NORTH_LOW[sensorIndex])
      return POL_NORTH;
  ...
  else
  {
    ...
    if (raw < THRESHOLD_SOUTH_LOW[sensorIndex])
      return POL_SOUTH;
    if (raw > THRESHOLD_NORTH_HIGH[sensorIndex])
      return POL_NORTH;
    return POL_UNCERTAIN;
  }
}
 ...
 */
bool updateRegistration(Polarity cur[3], uint16_t raw[3])
{
  bool allLocked = true;
    ...
    if (cur[i] == track[i].candidate && track[i].candidateCount >= CANDIDATE_STABILITY)
    {
      ...
      if (track[i].confirmCount >= CONFIRM_SAMPLES)
      {
        ...
        if (track[i].candidate == POL_SOUTH || track[i].candidate == POL_NORTH)
        {
          ...
          track[i].locked = true;
          track[i].lockedRaw = raw[i];  // Store raw ADC value at lock time
        }
        else
        {
          allLocked = false;
  ...
  if (hasNorth && registerDuration > NORTH_PATTERN_EARLY_LOCK_MS)
  {
    for (uint8_t i = 0; i < 3; i++)
    {
      if (!track[i].locked)
      {
        // Force-lock if sensor has established a NORTH candidate with any stability
        if (track[i].candidate == POL_NORTH && track[i].confirmCount >= 1)
        {
          DEBUG_PRINT("EARLY FORCE-LOCK sensor ");
          DEBUG_PRINT(i + 1);
          DEBUG_PRINTLN(" as NORTH (stable NORTH pattern)");
          track[i].confirmed = POL_NORTH;
          track[i].locked = true;
          track[i].lockedRaw = raw[i];
        }
        // Force-lock UNCERTAIN only if it has an established UNCERTAIN candidate (not just a transient reading)
        else if (track[i].candidate == POL_UNCERTAIN && track[i].confirmCount >= 3)
        {
          DEBUG_PRINT("EARLY FORCE-LOCK sensor ");
          DEBUG_PRINT(i + 1);
          DEBUG_PRINTLN(" as UNCERTAIN (stable UNCERTAIN in NORTH pattern)");
          track[i].confirmed = POL_UNCERTAIN;
          track[i].locked = true;
          track[i].lockedRaw = raw[i];
        }
  ...
  // AGGRESSIVE FALLBACK: Only kicks in if a sensor is genuinely stuck oscillating
  // for a long time, and only locks it to a value that has shown SOME stability —
  // never a single noisy instantaneous sample.
  int lockedCount = 0;
  for (uint8_t i = 0; i < 3; i++)
  {
    if (track[i].locked)
  ...
  if (lockedCount >= 1 && registerDuration > 500)  // was 200 — give slow sensors real time
  {
    for (uint8_t i = 0; i < 3; i++)
    {
      if (!track[i].locked)
      {
        // Only force-lock if this sensor has an established, non-UNCERTAIN candidate.
        // If it's still UNCERTAIN/unstable, wait for the FORCE_LOCK_TIMEOUT_MS path instead
        // of locking to a possibly-wrong instantaneous cur[i] value.
        if (track[i].candidate == POL_SOUTH || track[i].candidate == POL_NORTH)
        {
          DEBUG_PRINT("AGGRESSIVE FORCE-LOCK sensor ");
          DEBUG_PRINT(i + 1);
          DEBUG_PRINT(" as ");
          DEBUG_PRINTLN(track[i].candidate == POL_NORTH ? "NORTH" : "SOUTH");
          track[i].confirmed = track[i].candidate;
          track[i].locked = true;
          track[i].lockedRaw = raw[i];
        }
        // else: leave it unlocked — let normal confirm logic or the
        // FORCE_LOCK_TIMEOUT_MS path (which requires confirmCount >= 5) handle it
      }
    }
  }
  ...
  if (registerDuration > FORCE_LOCK_TIMEOUT_MS)
  {
    for (uint8_t i = 0; i < 3; i++)
    {
      if (!track[i].locked &&
          ...
          track[i].confirmCount >= 5)
      {
        DEBUG_PRINT("FORCE-LOCKING sensor ");
        DEBUG_PRINT(i + 1);
        DEBUG_PRINTLN(" as UNCERTAIN (stable timeout)");
        track[i].confirmed = POL_UNCERTAIN;
        track[i].locked = true;
        track[i].lockedRaw = raw[i];
      }
 ...
 * full board reset.
 * 
 * ENHANCED: Also checks raw ADC values against the LOCKED raw values to detect
 * piece removal even when hysteresis keeps polarity classification stuck.
 * This handles SLOW removal (gradual distance increase) where ADC drifts slowly.
 * 
 * @param cur Current polarity readings from all 3 sensors
 * @param raw Current raw ADC values from all 3 sensors
...
bool checkLockIntegrity(Polarity cur[3], uint16_t raw[3])
{
  ...
  for (uint8_t i = 0; i < 3; i++)
  {
    ...
    // CRITICAL: Compare current raw ADC against LOCKED raw value
    // This catches slow piece removal where ADC drifts gradually
    // Example: NORTH locked at 2600, slow removal: 2600→2550→2500→2450...
    // Increased threshold to 300 to tolerate ADC noise (8-sample averaging still allows ~20 unit variance)
    if (abs((int16_t)raw[i] - (int16_t)track[i].lockedRaw) > 300)
    {
      mismatchDetected = true;
      break;
    }
  ...
  if (mismatchDetected)
  {
    ...
    if (millis() - lockMismatchStartMs >= LOCK_MISMATCH_DEBOUNCE_MS)
    {
      ...
      DEBUG_PRINTLN(">>> RESETTING TO IDLE <<<\n");
      resetAllTracks();
      detectState = STATE_IDLE;
      stateEnteredAtMs = millis();
      isRegistered = false;
      isCorrectPlacement = false;
      lockedResultsRegistered = false;
      lockedResultsCorrect = false;
      return true;
    }
  }
  else
 ...
 * 
 * CRITICAL: FastLED.show() disables interrupts for ~600us, blocking I2C communication.
 * This function rate-limits updates to minimize I2C interference.
 * 
 * LED behavior:
 ...
 * 
 * NOTE: I2C fault does NOT block ring LED animations - diagnostic LEDs indicate I2C fault.
 */
void updateRingLeds(GameState gs, DetectState ds, bool registered, bool correct, bool i2cFault)
{
  // Rate limit LED updates to reduce I2C blocking from FastLED.show()
  // FastLED.show() disables interrupts, corrupting I2C if Teensy reads during update
  static uint32_t lastLedUpdateMs = 0;
  const uint32_t LED_UPDATE_INTERVAL_MS = 50; // Update every 50ms max
  
  if (millis() - lastLedUpdateMs < LED_UPDATE_INTERVAL_MS)
  {
    return; // Skip this update to keep I2C responsive
  }
  lastLedUpdateMs = millis();
  
  // RESULTS state shows final colors using LOCKED state (prevents flickering)
  if (gs == GAME_RESULTS)
  {
    if (!lockedResultsRegistered)
    {
      setLEDsBreathing(CRGB::White, 20, 150, 2000); // Empty slot
    }
    else if (lockedResultsCorrect)
    {
 ...
 *   [8-9]   S1 raw ADC:  uint16_t (high byte, low byte)
 *   [10-11] S2 raw ADC:  uint16_t (high byte, low byte)
 *   [12-13] S3 raw ADC:  uint16_t (high byte, low byte)
 */
void writeTxBuf(DetectState st, Polarity p0, Polarity p1, Polarity p2, uint16_t r0, uint16_t r1, uint16_t r2)
{
  noInterrupts();
  txBuf[0] = (uint8_t)st;
  txBuf[1] = (uint8_t)p0;
  ...
  // Pack raw ADC values as 16-bit big-endian (high byte first)
  txBuf[8]  = (uint8_t)(r0 >> 8);
  txBuf[9]  = (uint8_t)(r0 & 0xFF);
  txBuf[10] = (uint8_t)(r1 >> 8);
  txBuf[11] = (uint8_t)(r1 & 0xFF);
  txBuf[12] = (uint8_t)(r2 >> 8);
  txBuf[13] = (uint8_t)(r2 & 0xFF);
  interrupts();
}
  ...
  if (numBytes >= 1)
  {
        ...
        receivedState <= (uint8_t)GAME_RESULTS)
    {
      ...
      if (newGameState != gameState)
      {
        ...
        // CRITICAL: When entering RESULTS, ALWAYS recapture current state
        // This ensures piece swaps during previous RESULTS display are properly re-evaluated
        // Bug fix: Previously kept stale values if isRegistered was true, causing
        // incorrect correctness display when pieces were swapped during RESULTS state
        if (gameState == GAME_RESULTS)
        {
          lockedResultsRegistered = isRegistered;
          lockedResultsCorrect = isRegistered ? isCorrectPlacement : false;
          
          DEBUG_PRINT(">>> RESULTS LOCKED: ");
          DEBUG_PRINT(lockedResultsRegistered ? "Registered, " : "Not Registered, ");
          DEBUG_PRINTLN(lockedResultsCorrect ? "Correct" : "Incorrect");
        }
        
        DEBUG_PRINT(">>> GAME STATE CHANGED: ");
        DEBUG_IF(
          switch (gameState)
          {
...
void setup()
{
  // Initialize serial communication (for diagnostic commands, even if verbose debug disabled)
  ...
  DEBUG_PRINTLN("Waiting for power stabilization...");
  delay(500);

  // Initialize external LED ring
  FastLED.addLeds<WS2812, LED_PIN, GRB>(leds, NUM_LEDS);
  FastLED.setMaxPowerInVoltsAndMilliamps(3.3, 500);
  // Set max refresh rate to prevent I2C blocking - FastLED.show() won't run more than once per 20ms
  FastLED.setMaxRefreshRate(50); // Max 50 FPS to reduce interrupt blocking
  setLEDs(CRGB::Black);

  // Initialize sensor tracking and statep
  resetAllTracks();
  detectState = STATE_IDLE;
  ...
  DEBUG_IF(Serial.println(i2cAddress, HEX);)
  DEBUG_PRINT("Expected Pattern: ");
  DEBUG_IF(printI2Caddress();)
  DEBUG_PRINT("Configuration: ");
  DEBUG_PRINT(ADC_SAMPLES);
  DEBUG_PRINT(" ADC samples ("  );
  DEBUG_PRINT(ADC_SETTLE_DELAY_US);
  DEBUG_PRINT("us settle), ");
  DEBUG_PRINT(CONFIRM_SAMPLES);
  DEBUG_PRINT(" confirm samples, ");
  DEBUG_PRINT(DEBOUNCE_DELAY_MS);
  DEBUG_PRINTLN("ms debounce");
  DEBUG_PRINTLN("⚠️  THRESHOLDS & PATTERNS FLAGGED FOR RECALIBRATION ⚠️");
  DEBUG_PRINTLN("============================================\n");
  ...
  Serial.println("Per-sensor UNCERTAIN ranges (between SOUTH_HIGH and NORTH_LOW):");
  Serial.print("  S1: ");
  Serial.print(THRESHOLD_SOUTH_HIGH[0]);
  Serial.print("-");
  Serial.println(THRESHOLD_NORTH_LOW[0]);
  Serial.print("  S2: ");
  Serial.print(THRESHOLD_SOUTH_HIGH[1]);
  Serial.print("-");
  Serial.println(THRESHOLD_NORTH_LOW[1]);
  Serial.print("  S3: ");
  Serial.print(THRESHOLD_SOUTH_HIGH[2]);
  Serial.print("-");
  Serial.println(THRESHOLD_NORTH_LOW[2]); 
  // Warn if any sensor is outside expected UNCERTAIN range
  bool baseline_ok = true;
  if (baseline_s1 < THRESHOLD_SOUTH_HIGH[0] || baseline_s1 > THRESHOLD_NORTH_LOW[0])
  {
    Serial.println("⚠️  WARNING: S1 baseline outside UNCERTAIN range!");
    baseline_ok = false;
  }
  if (baseline_s2 < THRESHOLD_SOUTH_HIGH[1] || baseline_s2 > THRESHOLD_NORTH_LOW[1])
  {
    Serial.println("⚠️  WARNING: S2 baseline outside UNCERTAIN range!");
    baseline_ok = false;ss
  }
  if (baseline_s3 < THRESHOLD_SOUTH_HIGH[2] || baseline_s3 > THRESHOLD_NORTH_LOW[2])
  {
...
void loop()
{
  ...
  for (uint8_t i = 0; i < 3; i++)
  {
    ...
    cur[i] = classifyWithHysteresis(raw[i], track[i].lastReading, i);
  ...
  case STATE_DEBOUNCING:
    ...
    if (allUncertain)
    {
      ...
      if (Serial) Serial.println(">>> FALSE TRIGGER - BACK TO IDLE <<<");
      resetAllTracks();
      detectState = STATE_IDLE;
      stateEnteredAtMs = millis();
      isRegistered = false;
    ...
    else if (millis() - stateEnteredAtMs >= DEBOUNCE_DELAY_MS)
    {
      ...
      if (Serial) Serial.println(">>> DEBOUNCE COMPLETE - STARTING REGISTRATION <<<");
      resetAllTracks();
  ...
  case STATE_REGISTERING:
    ...
    if (allUncertain)
    {
      ...
      DEBUG_PRINTLN(">>> OBJECT REMOVED BEFORE CONFIRMATION - BACK TO IDLE <<<");
      resetAllTracks();
      detectState = STATE_IDLE;
      stateEnteredAtMs = millis();
      isRegistered = false;
    ...
    {
      bool allLocked = updateRegistration(cur, raw);
      uint32_t registerDuration = millis() - stateEnteredAtMs;
      ...
      if (allLocked && registerDuration >= MIN_REGISTERING_DISPLAY_MS)
      {
        ...
        stateEnteredAtMs = millis();
        isRegistered = true;  // CRITICAL: Set immediately when locking
        isCorrectPlacement = correct;
        
        // CRITICAL: Capture results NOW for RESULTS state display
        // This prevents piece removal between lock and RESULTS from showing wrong color
        lockedResultsRegistered = true;
        lockedResultsCorrect = correct;

        // Minimal lock message
        DEBUG_PRINT("LOCKED 0x");
        DEBUG_IF(Serial.print(i2cAddress, HEX);)
        DEBUG_PRINT(": ");
        DEBUG_PRINTLN(correct ? "CORRECT" : "INCORRECT");
        ...
        Serial.print("DEBUG: correct=");
        Serial.print(correct);
        Serial.print(", isCorrectPlacement=");
        Serial.print(isCorrectPlacement);
        Serial.print(", lockedResultsCorrect=");
        Serial.print(lockedResultsCorrect);
        Serial.print(", detectState=");
        Serial.println(detectState == STATE_CORRECT ? "CORRECT" : "INCORRECT");
        Serial.print("I2C BUFFER: [0]=");
        Serial.print(detectState);
        Serial.print(" [6]=");
        Serial.print(isRegistered ? 1 : 0);
        Serial.print(" [7]=");
        Serial.println(isCorrectPlacement ? 1 : 0);
        #endif
      }
  ...
  case STATE_INCORRECT:
    ...
    // CRITICAL: During GAME_RESULTS or GAME_ACTIVE, keep pieces locked
    // GAME_RESULTS: prevents flickering when pieces removed during results display
    // GAME_ACTIVE: prevents unlock/relock cycles from ADC noise during gameplay
    // GAME_PRE_RESULTS: also keep locked (preparing for results)
    if (gameState == GAME_RESULTS || gameState == GAME_ACTIVE || gameState == GAME_PRE_RESULTS)
    {
      // ENHANCED REMOVAL DETECTION: Check raw ADC values, not just polarity classification
      // This fixes Wind removal bug where NORTH sensors drop to SOUTH (not UNCERTAIN) due to baseline drift
      // Algorithm: Piece is removed if ALL previously-magnetic sensors have returned to baseline range
      bool pieceRemoved = true;
      for (uint8_t i = 0; i < 3; i++)
      {
        if (track[i].confirmed == POL_NORTH)
        {
          // Sensor was locked as NORTH - check if still reading NORTH range
          if (raw[i] > THRESHOLD_NORTH_LOW[i])
          {
            pieceRemoved = false; // Still in NORTH range
            break;
          }
        }
        else if (track[i].confirmed == POL_SOUTH)
        {
          // Sensor was locked as SOUTH - check if still reading SOUTH range
          if (raw[i] < THRESHOLD_SOUTH_HIGH[i])
          {
            pieceRemoved = false; // Still in SOUTH range
            break;
          }
        }
        // UNCERTAIN sensors are ignored (already at baseline when locked)
      }
      
      if (pieceRemoved)
      {
        DEBUG_PRINTLN(">>> MAGNET REMOVED - BACK TO IDLE <<<\n");
        resetAllTracks();
        detectState = STATE_IDLE;
        stateEnteredAtMs = millis();
        isRegistered = false;
      ...
      // Otherwise stay locked - ignore ADC drift during gameplay
      break;
    }
    
    // GAME_READY_IDLE state: Check lock integrity (user might be adjusting pieces)
    // This allows pieces to unlock if user swaps/adjusts them before starting game
    ...
    // DISABLED during GAME_ACTIVE/PRE_RESULTS/RESULTS - only active in READY_IDLE
    // This prevents false unlocks during normal gameplay
    if (gameState == GAME_READY_IDLE && millis() - lastRawChangeMs > LOCK_NOCHANGE_TIMEOUT_MS)
    {
      ...
      // Use per-sensor thresholds since each sensor has different characteristics
      for (uint8_t i = 0; i < 3; i++)
      {
        if ((raw[i] > THRESHOLD_NORTH_LOW[i] && raw[i] < THRESHOLD_NORTH_HIGH[i]) ||
            (raw[i] < THRESHOLD_SOUTH_HIGH[i] && raw[i] > THRESHOLD_SOUTH_LOW[i]))
        {
      ...
      if (stuckOutsideBaseline)
      {
        ...
        DEBUG_PRINTLN(">>> (Power supply voltage may have drifted) <<<\n");
        resetAllTracks();
        detectState = STATE_IDLE;
        stateEnteredAtMs = millis();
        isRegistered = false;
  ...
  // CRITICAL: Update isRegistered AFTER state machine runs to prevent stale value
  ...
  writeTxBuf(detectState, txS1, txS2, txS3, raw[0], raw[1], raw[2]);

  // ── Process Serial Commands ─────────────────────────────────────────────────
  // Check for diagnostic command input
  if (Serial && Serial.available())
  {
    ...
    if (cmd == 's' || cmd == 'S') // Status poll command
    {
      const char *POL[] = {"X", "S", "N"};
      Serial.print("STATUS: ");
      Serial.print(isCorrectPlacement ? "CORRECT" : "INCORRECT");
      Serial.print(" | s1=");
      Serial.print(POL[track[0].confirmed]); // S1 (middle)
      Serial.print(" s2=");
      Serial.print(POL[track[1].confirmed]); // S2 (left)
      Serial.print(" s3=");
      Serial.print(POL[track[2].confirmed]); // S3 (right)
      Serial.print(" | RAW: s1=");
      Serial.print(raw[0]); // S1 (middle)
      Serial.print(" s2=");
      Serial.print(raw[1]); // S2 (left)
      Serial.print(" s3=");
      Serial.print(raw[2]); // S3 (right)
      Serial.print(" | Reg:");
      Serial.print(isRegistered ? "Y" : "N");
      Serial.print(" | State:");
      if (detectState == STATE_IDLE) Serial.print("IDLE");
      else if (detectState == STATE_DEBOUNCING) Serial.print("DEB");
      else if (detectState == STATE_REGISTERING) Serial.print("REG");
      else if (detectState == STATE_CORRECT) Serial.print("CORRECT");
      else if (detectState == STATE_INCORRECT) Serial.print("INCORR");
      Serial.print(" | THR: s1S<");
      Serial.print(THRESHOLD_SOUTH_HIGH[0]);
      Serial.print(",s2S<");
      Serial.print(THRESHOLD_SOUTH_HIGH[1]);
      Serial.print(",s3S<");
  ...
  // ── Update LEDs ─────────────────────────────────────────────────────────────
  // CRITICAL: Delay after I2C buffer update to give Teensy time to read before
  // FastLED.show() disables interrupts (~600μs). This prevents I2C lockout.
  // Teensy I2C read takes ~100-200μs, so 1ms window ensures reliable reads.
  delay(1); // 1ms window for I2C read before LED update blocks interrupts
