    ...
    public class Sandbox : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (initialCalibrationComplete)
            {
                ...
                if (!initialStart)
                {
                    menuManager.OpenWaterSimulation();
                    modeSelector.EnableWaterSimulation();
                    initialStart = true;
                }
                ...
                if (Input.GetKeyDown("x"))
                {
                    if (initialStart)
                    {
                        if (!toggleWaterSim)
                        {
                            modeSelector.DisableCurrentMode();
                            menuManager.OpenMainMenu();
                            toggleWaterSim = true;
                        }
                        else if (toggleWaterSim)
                        {
                            menuManager.OpenWaterSimulation();
                            modeSelector.EnableWaterSimulation();
                            toggleWaterSim = false;
                        }
                    }
                }
        ...
        public void UI_ChangeMajorContourSpacing(float spacing)
        {
            ...
            //NormalMaterial.SetFloat("_ContourStride", (15.0f / 2.0f));
        }
        public void UI_ChangeMinorContourAmount(float amount)
        {
            ...
            //NormalMaterial.SetInt("_MinorContours", 0);
        }
        public void UI_ChangeContourThickness(float thickness)
        {
            ...
            //NormalMaterial.SetFloat("_ContourWidth", (0.41f * 30));
        }
