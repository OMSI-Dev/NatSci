...
namespace ARSandbox.WaterSimulation
{
    public class WaterSimulation : MonoBehaviour
    {
        ...
        public AudioSource droughtAudioSource;

        private float audioTimeout = 0.5f;
        ...
        private void OnGesturesReady()
        {
            ...
            foreach (HandInputGesture gesture in HandInput.GetCurrentGestures())
            {
                if (!gesture.OutOfBounds)
                {
                    if (!Physics.CheckSphere(gesture.WorldPosition + new Vector3(0, 0, -5), 1.0f))
                    {
                        ...
                        {
                            rainAudioSource.clip = rainAudio;
                            rainAudioSource.loop = true;
                            rainAudioSource.volume = 1f;
            ...
            if (rainAudioSource.isPlaying && !isAudioTimerRunning)
            {
                StartCoroutine(FadeOutAudio(rainAudioSource, 1.5f));
            }

            if(isAudioTimerRunning)
            {
                if(audioTimer > audioTimeout)
                {
        ...
        private IEnumerator FadeOutAudio(AudioSource source, float fadeTime)
        {
            ...
            while(timePassed < fadeTime)
            {
                ...
                source.volume = Mathf.Lerp(startVolume, 0, timePassed / 1.5f);
                yield return null;
            }
            source.Stop();
            source.volume = 1f;
        ...
        private void DestroyWaterDroplets()
        {
            foreach (WaterDroplet droplet in waterDroplets)
            {
                if (Input.GetKey("up"))
                {
                    droughtAudioSource.clip = droughtAudio;
                    droughtAudioSource.loop = false;
                    droughtAudioSource.volume = 1f;
                    droughtAudioSource.Play();
                    StartCoroutine(FadeOutAudio(droughtAudioSource, 3.5f));
